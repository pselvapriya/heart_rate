...
Utils.prototype.getEmailBody = function(content) {
    var header =
        '<div style=""><div style="text-align:center;border-bottom:2px solid #4e5467;padding: 10px"><small>A Product of Anyware</small><h5 style="font-weight: bold;letter-spacing: 5px;margin:0px;font-size: 1.2rem;color:#4b81c2;padding-left: 10px">Quality Check</h5></div>';
    var footer =
        '<div style="text-align:center;border-top:2px solid #4e5467; padding: 10px;background-color:#4e5467;color:#fff">&copy; 2020 All rights reserved. Powered by Boodskap Inc.,</div></div>';
    ...
    return (
        ...
        content +
        '<p style="color:#666;margin: 20px 0px">Regards,<br><b>Quality Check</b></p></div></div>' +
        footer
    );
};
Utils.prototype.getTwilioAuth = function() {
    const self = this;
    let auth =
        self.app.conf.settings.twilio.account_sid +
        ":" +
        self.app.conf.settings.twilio.auth_token;
...
Utils.prototype.getErrorDesc = function(code) {
    ...
    for (var i = 0; i < ErrorCode.length; i++) {
        if (code * 1 === ErrorCode[i].code * 1) {
            str = ErrorCode[i]["secondary_message"] ?
                ErrorCode[i]["secondary_message"] :
                ErrorCode[i]["message"];
            break;
        }
    }
